...
- ★★**나눌 수 없으면 `bash -c '…'` 로 감싸라 — `glab` 만의 이야기가 아니다.** 울타리 밖 목록은 **첫 낱말만** 본다: `cd {폴더} && glab …`·`export …; npm test` 는 목록에 안 걸려 갇히고, 그러면 AI 가 울타리를 꺼서 **사람에게 확인 창이 뜬다**(2026-09-09 실측: bnsone 하루 281건).
