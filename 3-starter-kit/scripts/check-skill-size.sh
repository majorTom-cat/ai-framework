...
#
# ★차단형이다(넘으면 exit 1) — 2026-09-09(저녁3) 오너 결정 «사람한테 묻지 말고 기계가 막아라».
#   원래는 경고형(exit 0)이었다. 그런데 «넘는지»는 **사람이 ▶ 를 눌러서는 절대 알 수 없는 값**이고,
#   실제로 09-09 에 스킬 셋이 조용히 한도를 넘어 뒤쪽이 통째로 안 읽히고 있었다(`/done` 은 아무도 몰랐다).
#   경고형인 동안은 CI 가 초록이라 그대로 머지됐다 — 그래서 사람 클릭 대신 이 검사기를 방어선으로 삼는다.
# 넘는 스킬의 처방은 둘: ① 뒤쪽 절을 보조 파일로 빼고 ② **보조 파일 안내를 맨 앞에** 둔다(뒤에 두면 포인터째로 잘린다).
set -u
