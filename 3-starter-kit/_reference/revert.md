...
> 자동 로드 ❌. CLAUDE.md '금지'·'스키마·데이터' 절이 이 파일을 가리킨다. 규칙 명령문은 그쪽에, 절차·근거는 여기.
...
5. 커밋 메시지 `#카드 롤백 — 되돌리는 새 마이그레이션` → MR은 **고위험(DB 마이그레이션)** = 경고 레인: AI 경고 리뷰 + 증적 3종 후 `high-risk-gate` 수동 실행. push 때 훅이 공통 영역 ask를 낸다(정상 동작이다 — 내용 확인 후 진행)
