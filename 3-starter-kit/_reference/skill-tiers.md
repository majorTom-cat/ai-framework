...
> ⚠️**2026-09-16 개정 — 이 표는 저장 워크플로 `merge-review` 를 쓸 때(위험 변경)만 적용된다.** 머지 전 리뷰의 기본은 **독립 리뷰어 1명 + «직접 실행해 재현하라»**다(오너 결정 · 정본 = `done/단계상세.md` §9). `*.md` 만 바뀐 변경은 위험 변경이 아니므로 이 표의 `medium` 행까지 올 일이 없다.
