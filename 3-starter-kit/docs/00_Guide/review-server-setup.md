...
> **절차·경고의 정본 = 이 파일(킷 배포판)**. 사내 실측 상수(서버 주소·레지스트리·계정·경로)만 프레임워크 repo `4-reference/review-server-setup.md` 표를 참조한다. ⚠️2026-09-09 정정: 예전엔 이 줄이 반대로 «4-reference 판이 정본» 이라고 적혀 있었는데, 실제로는 그쪽이 08-03 에 멈췄고 이 파일이 08-06·08-14 규칙(⓪ nslookup 선확인·③ git clone·⑤ RBAC 택1·⑥ 빈 커밋 금지)을 더 받았다 — 낡은 쪽으로 사람을 보내고 있었다.
> **자동 로드 안 됨.** main 머지 시 자동 배포되는 리뷰 환경(검수 서버)을 사내 인프라에 **한 번** 세팅하는 방법. ★**실행 대상 = 실 배포 인프라를 붙이는 repo뿐** — 검수 서버 없이 도는 repo(리허설·골격 단계)에서 아래 ⑤(deploy-review 스텁 전환)만 따라 하면 CLAUDE.md '배포' 서술과 충돌한다. 그런 repo에선 이 문서를 실행하지 마라. 일상 흐름(`/done` → 머지 → 자동 배포 → 검수 요청)은 스킬·가이드가 담당한다. 시안 허브(`sian-hub-setup.md`)와 별개 — 허브는 docs 정적 파일, 검수 서버는 **실행되는 앱**.
...
### ④ 마이그레이션·시드 = 앱 엔트리포인트
- CI에 별도 migrate Job을 만들지 않는다(CI 계정 권한을 최소로 유지). **이미지 엔트리포인트가 파드 기동마다 forward-only·멱등 마이그레이션 + 멱등 시드를 실행**한다 — 그 repo의 마이그레이션 러너(폴더·명령은 스택 값 — `rules/migrations.md`가 정본)를 여기 연결. 온디맨드 전체 리셋 = DB 파드·PVC 재생성 스크립트 1개.
