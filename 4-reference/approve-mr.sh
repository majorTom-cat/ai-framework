...
#   옛 파이프라인의 종결초록을 보고 머지를 시도해 405 가 난다.
#   ★2026-09-09 정정: 새 파이프라인은 **빈 커밋이 아니라** `POST merge_requests/<iid>/pipelines` 로 만든다(빈 커밋은 change-summary 류 잡을 헛돌린다).
#   ★405 의 다른 원인 = **main 과 충돌**. 그때는 파이프라인을 아무리 기다려도 안 풀린다 — `git merge origin/main` 으로 풀고 새 파이프라인을 돌려라.
...
PIPE=$(head_pipe "$MR")
# ★새 파이프라인 생성 직후엔 head_pipeline 이 잠깐 null 이다 — 최대 6회(1분) 재시도 (8-14 실측: 즉시 exit 1 로 죽음)
