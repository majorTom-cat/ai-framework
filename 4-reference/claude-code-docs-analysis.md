...
- **작성 원칙(공식):** 500줄 이하, 참조 파일은 1단계 깊이까지, 체크리스트를 응답에 복사시키는 패턴 공식 지원, 자유도 조절(취약한 단계는 "정확히 이 명령만"), 검증 루프("통과할 때까지"), 진행 중 지침은 "1회 단계"가 아니라 "상시 지침"으로 서술(스킬 본문은 재독되지 않음). compaction 시 **각 스킬의 «앞 5,000토큰»만 남기고 뒤를 자른다**(드롭이 아니라 절단 — 2026-09-07 공식 재확인: "keeping the first 5,000 tokens of each"). 재부착 스킬 **전체 합산 예산은 25,000토큰**이고 **최근 호출 순으로 채우므로, 한 세션에서 스킬을 많이 부르면 오래된 것은 통째로 빠진다.** ★**보조 파일은 이 예산에 안 든다** — 읽을 때만 로드되므로 "long reference material costs almost nothing until you need it". 그래서 **본문이 5,000토큰을 넘으면 «뒤쪽 절»을 보조 파일로 빼는 것이 정답**이다(잘려서 안 읽히느니 포인터로 읽히는 편이 낫다 — 보조 파일을 실제로 찾아 읽는 것은 2026-09-07 프로브로 실증).
