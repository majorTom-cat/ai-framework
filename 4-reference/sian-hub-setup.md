...
```
(사내 예: 네임스페이스 `ssd-cicd` — GitLab이 깔린 곳. 여기 두면 기존 nginx-ingress·인증서를 재사용.)
...
> 사내 실측값(파일럿): 네임스페이스 `ssd-cicd`, 허브호스트 `sian.bns.co.kr`, 첫 프로젝트 경로 `/ai-framework-pilot/`, HTTP(전용 인증서 없음). 러너/아티팩트 버그 해결 시 GitLab Pages로 이관 검토.
