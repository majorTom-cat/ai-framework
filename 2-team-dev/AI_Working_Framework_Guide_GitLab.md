...
| 양식 **"검수: @누구"** | 배포 화면을 확인할 사람. ⚠️**내가 구현할 카드면 비우지 말고 다른 사람 1명 지정**(비워두면 만든 사람이 스스로 검수하게 돼 "내가 만든 걸 내가 합격 처리"가 된다) |
...
> 💡 화면에 안 나오는 작업(내부 정리, 문서 등)은 검수요청 없이 **셀프완료**한다: 카드에 **완료 증거(실행 결과·링크) + `셀프완료` 표식을 댓글**로 남기고 닫는다(카드 작성자가 남이면 완료 통보 @멘션). MR Description의 `Closes #12`(머지 시 자동 닫힘)는 **이 증거 댓글을 남긴 경우에만** 쓴다 — 댓글 없이 자동으로 닫히면 완료 근거가 안 남는다. 검수가 필요한 카드에는 쓰지 말 것 (검수를 건너뛰게 됨).

---

## 5. 검수 — 배정받은 사람이 확인하고 완료 처리하는 법

> **검수 전담자는 따로 없다.** 검수는 **카드의 "검수" 필드에 지정된 사람**(비면 카드 작성자)이 한다 — 개발자든 비개발자든. 기능·화면 카드는 주로 기획/검수가, 개발자가 낸 카드는 다른 개발자가 맡는 식으로 카드마다 배정된다. 아래는 "검수를 맡은 사람"이 하는 일이다.
...
2. **카드의 "검수 방법" 칸을 먼저 읽고**, **검수 서버 URL**을 브라우저로 열어 그 기능을 실제로 사용해 본다. **통과 기준 3개**:
   ...
   - 화면 하단의 버전 표시(커밋·배포 시각)로 "내가 보는 게 최신인지" 확인 가능
   - **검수는 검수자가 개발자든 비개발자든 "화면 기준"이다** — 코드 품질(로직·보안·중복)은 CI·AI 리뷰·(고위험이면)동료 MR 승인이 이미 봤으니, 검수는 "카드가 원한 대로 화면에서 되나"만 본다. 개발자 검수자라고 코드까지 다시 볼 필요 없다
3. 판정:

| 판정 | 하는 일 |
| --- | --- |
| ✅ 이상 없음 | 카드 열고 아래 **Close issue** 버튼 클릭 → 카드가 완료(Closed) 열로 이동. 끝 |
...
- **합격시킨 게 나중에 문제로 밝혀지면?** — 검수는 **화면 확인이지 품질 보증이 아니다.** 이후 발견된 결함은 문책이 아니라 **새 카드**로 처리된다(코드 품질은 CI·AI 리뷰·동료 승인이 이미 봤다)
...
2. **라벨·보드 확인**: 상태(`진행중`·`검수요청`)·`무효`(기획 변경으로 닫힌 카드)·`대기(불명확)`(수용 기준 모호)·`고위험`(인증·결제·DB 마이그레이션·shared 승격·핫스팟〔전원이 함께 쓰는 예민한 파일 — 설정·라우팅 등〕 — 머지 전 동료 1명 승인)·모듈 라벨이 있고, 보드에 대기/진행중/검수요청/완료 4열이 보이면 OK. ★라벨이 없으면 카드 등록·이동이 **조용히 실패**한다(GitLab은 없는 라벨을 자동 생성 안 함) — 빠졌으면 `/setup-gitlab` 재실행
   - ⚠️**`고위험` 승인의 세부 규칙**(정본 = repo `CLAUDE.md` '머지 등급' 절): **승인자는 개발자끼리 서로 — 공통 개발자는 승인자가 아니다**(개발자 1명뿐일 때만 예외. 공통 개발자에게 몰리면 그가 전담 리뷰어가 되어 이 규칙이 막으려던 병목이 그대로 생긴다, 2026-08-06 실측). 자기 코드 자기 승인 금지. 동료 승인이 불가능할 때(혼자 작업·전원 부재·긴급)만 **셀프 승인**을 인정하되 **증적 3종**(①fresh-context AI 리뷰 ②본인 diff 확인 ③카드 댓글에 "셀프 승인"+사유+리뷰 결과+확인 범위)이 다 남아야 하고, 조용히 머지하면 무승인이다.
