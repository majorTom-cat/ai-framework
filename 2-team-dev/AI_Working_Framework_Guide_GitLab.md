...
| ⑤ 정리 | 연습 카드에 라벨 없이 "연습이었음" 댓글 한 줄을 달고 닫는다(Close). **삭제하지 않고 닫는 것**, 이게 이 팀의 규칙이다 | §2-2 치트시트 |
...
4. 맨 아래 **Commit message** 한 줄(`docs: 프로젝트 차터 등록`) → **Commit changes** 클릭. 그 다음은 아래 그림의 두 갈래 중 하나로 흘러간다:
...
> ⚠️ ②의 길은 **Merge까지가 한 세트** — 중간에 창을 닫으면 올린 문서가 반영되지 않은 채 남는다(Web IDE로 올릴 때도 같은 흐름). **빨간불(실패)이면 Merge 누르지 말고** 공통 개발자를 호출한다.
...
> 3. 왼쪽 **Source Control**(변경 목록, `Ctrl+Shift+G`)에서 한 번에 **Commit** — 이후는 위 그림 ②의 흐름 그대로(Merge까지)
>
...
| 양식 **"위험도"** | 로그인·결제·데이터 구조 변경(DB 마이그레이션)·공통 부품(shared)을 건드리면 `고위험` 라벨(대표 예 — 전체 기준의 정본 = repo `CLAUDE.md` '머지 등급' 절). **잘 모르겠으면 비워두세요 — 개발자가 판단해 붙입니다** |
...
**등록만 하면 끝이다. 개발자를 찾아가 말할 필요가 없다.** 개발자마다 AI 비서(Claude)가 붙어 있고, 세션을 켤 때와 작업 하나를 끝낼 때마다 보드를 확인해 새 카드를 요약 브리핑하기 때문이다 — 카드 하나가 반나절~이틀 크기라 **보통 몇 시간 안에 닿는다.**
급할 때만 카드 **댓글에 `@개발자아이디` 멘션**(알림이 감) + 팀 채팅으로 호출한다 — 단, 하던 일을 멈추고 이걸 먼저 할지는 개발자가 결정한다.
...
> 원칙: **변경은 반드시 문서(docs/)에 반영한다 — 진행중 작업에 영향이면 해당 카드 댓글도(위 표).** 말로만 전하면 개발자의 AI가 모르는 채로 옛 문서대로 만든다.

> 💡 **옛 문서와 새 문서의 내용이 어긋나면** — **더 최신 문서가 정본**입니다. AI는 **여러분이 올린 원문에는 아무것도 쓰지 않고**(안내 표시도 붙이지 않아요), 자기 요약 문서(`_digest.md`)에 기록한 뒤 멘션으로 알려드려요. 원문을 고칠지는 원하실 때 직접 정하시면 됩니다.
...
| **실수로 닫았을 때** | 같은 자리의 **Reopen issue** — 정말 끝난 카드의 후속 일은 새 카드(기준 정본 = §2-1 표) |
...
- 머지 = 곧 **검수 서버 자동 배포** — 개발자 본인이 **배포 화면을 1분 직접 확인**한 뒤에야 검수요청으로 넘어간다 (검수 서버가 아직 없는 설치 단계면 로컬 기동 화면으로 대체 — 확인 없이 넘어가지 않는 것이 규칙의 본체다)
- 공통 영역(shared 등)을 건드린 변경이면 **코드에 손대기 전에** 채팅 공지가 이미 있었어야 한다(머지 직전이 아니라)

> 💡 화면에 안 나오는 작업(내부 정리, 문서 등)은 검수요청 없이 **셀프완료**한다 — 증거 댓글·`셀프완료` 라벨 전환·닫기까지의 **절차 정본 = `/done` 8단계**(카드 작성자가 남이면 완료 통보 @멘션).
>
> ⚠️ **MR·커밋에 `Closes #12`(`Fixes`·`Resolves` 포함)는 쓰지 않는다 — 예외 없다(셀프완료도).** 머지 순간 카드가 자동으로 닫혀 증거·검수를 통째로 건너뛴다(실사고 있음 — 상세 정본 = `/done`). MR 본문에는 `관련: #12`로 **링크만** 건다.
...
2. **카드의 "검수 방법" 칸을 먼저 읽고**, **검수 서버 URL**을 브라우저로 열어 그 기능을 실제로 사용해 본다. **통과 기준 3개**:
   ...
   - **"내가 보는 게 최신인가" 확인**: 화면 맨 아래에 **커밋(7자리)·기동 시각**이 작게 표시된다.
     이 커밋이 카드에 걸린 MR의 머지 커밋과 같으면 최신 — 다르면 아직 배포 전이니 잠시 뒤 새로고침, 계속 다르거나 커밋 자리에 `dev`가 보이면 공통 개발자 호출
...
| 판정 | 하는 일 |
| --- | --- |
| ✅ 이상 없음 | **내 Claude 세션에 "이상 없음"이라고 말하면 AI가 대신 닫아준다**(확인 댓글·라벨 정리까지 — 반드시 검수 담당인 당신 세션에서). 웹이 편하면 카드 열고 아래 **Close issue** 버튼 클릭 — 어느 쪽이든 카드가 완료(Closed) 열로 이동. 끝 |
| ❌ 문제 있음 | 카드에 **댓글**로 문제를 적고(화면/했던 것/기대/실제/스크린샷), 카드를 **진행중** 열로 되돌린다 |
| 🆕 다른 문제 발견 | 그 카드는 정상 종료하고, **새 카드**로 등록 (2절) — 한 카드에 여러 문제를 섞지 않는다 |
| 📝 "스펙과 달라진 점" 댓글이 있음 | 개발자가 **더 나은 방안으로 기획과 다르게 만든 것** — **여러분의 원문은 그대로**이고, 달라진 점은 그 댓글과 AI 요약 문서(`_digest.md`)에 적혀 있다. 화면과 댓글을 보고 판단한다. 동의하면 Close 하고 **원문을 새 내용으로 고칠지는 직접 정한다**, 아니면 댓글 "원래 기획대로 해주세요" + 진행중으로 되돌림. **기획의 최종 판단은 여전히 당신 몫이다** |
...
> 💡 변경이 크거나 팀과 논의가 필요하면, 문서만 고치지 말고 **카드도 한 장 남긴다**(§2) — 왜 바꿨는지가 기록으로 남고 담당 개발자에게 알림이 간다. (변경은 반드시 docs에 반영 — §2-1 끝의 원칙 그대로다.)

마지막으로, Merge 후 main에서 그 문서를 다시 열어 고친 내용이 보이는지 확인하세요(§1의 화면 확인과 같은 원리).

---

## 8. 공통 개발자 — 처음 한 번만 하는 보드 준비

이 가이드가 전제하는 설정 — **공통 개발자가 `/setup-gitlab` 한 번으로** 세팅한다(수동 클릭 불필요 — 라벨·설정 명세의 정본은 그 스킬). 아래는 무엇이 세팅되는지와 확인 기준이다.

1. **계정 초대**: Manage → Members → Invite members. 개발자는 `Developer`, **기획/디자이너는 `Maintainer`** — 이 팀은 기획/디자이너가 `docs/`에 직접 올리고 고치는 방식(§1·§7)이라 push 권한이 필요하다. "docs만" 규율과 백스톱(CE 경로 보호 한계 · CI `planner-guard` 사후 감지)의 배경·결정 정본 = [ADR-0001](/docs/adr/0001-비개발자-docs-직접푸시.md).
2. **라벨·보드 확인**: 상태(`진행중`·`검수요청`·`셀프완료`)·`무효`·`대기(불명확)`·`고위험`·`셀프승인`·`승인요청`·모듈 라벨(+`shared`·`docs`)이 있고, 보드에 대기/진행중/검수요청/완료 4열이 보이면 OK. ★라벨이 없으면 카드 등록·이동이 **조용히 실패**한다(§2 ⚠️와 같은 함정) — 빠졌으면 `/setup-gitlab` 재실행. `고위험` 경고 레인(셀프승인 라벨·증적 3종·동료 승인 선택)의 세부 규칙 정본 = repo `CLAUDE.md` '머지 등급' 절.
3. 팀원 전원에게 **즐겨찾기** 안내: 이슈 보드 · docs/ 폴더 · 검수 서버 URL · (화면 시안을 쓰면) **시안 허브 URL**
4. **(화면 시안을 쓸 프로젝트면) 시안 허브 세팅** — 비개발자가 화면 후보를 브라우저로 보고 고르게 하는 정적 서버. **인스턴스에 1회**만 띄우면 이후 프로젝트는 경로만 추가한다. 절차·매니페스트·트러블슈팅 정본 = [sian-hub-setup.md](sian-hub-setup.md). (사내 GitLab에 Pages가 정상이면 이 단계 대신 각 프로젝트 CI의 `pages` 잡을 켠다.)
...
| 완료된 카드를 다시 열 수 있나요 | **실수로 닫았을 때만** Reopen. 새 수정·개선이면 새 카드 — 기준 정본 = §2-1 표 |
