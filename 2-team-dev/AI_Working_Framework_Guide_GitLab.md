...
2. **라벨·보드 확인**: 상태(`진행중`·`검수요청`·**`셀프완료`**)·`무효`(기획 변경으로 닫힌 카드)·`대기(불명확)`(수용 기준 모호)·`고위험`(인증·결제·DB 마이그레이션·shared 승격·핫스팟〔전원이 함께 쓰는 예민한 파일 — 설정·라우팅 등〕 — 머지 전 AI 경고 리뷰+증적)·**`셀프승인`**(게이트 본인 통과 열쇠)·**`승인요청`**(동료 승인 대기 — 채팅 알림용)·모듈 라벨(+`shared`·`docs`)이 있고, 보드에 대기/진행중/검수요청/완료 4열이 보이면 OK. ★라벨이 없으면 카드 등록·이동이 **조용히 실패**한다(GitLab은 없는 라벨을 자동 생성 안 함) — 빠졌으면 `/setup-gitlab` 재실행
