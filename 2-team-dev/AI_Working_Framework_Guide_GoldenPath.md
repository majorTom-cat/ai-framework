...
- **사람 게이트는 최대 2개** (스펙 승인·계획 go) — 그 사이는 연속 실행. **작은 카드**(질문 없음·작업 3개 이하·스키마/공개 계약 불변·고위험 아님)는 둘을 묶어 **1회**로 선다(정본 = `/dev` 2단계 '묶음 게이트'). 성공 사례들의 공통 교훈: *"'계속할까요?' 남발이 시간을 죽인다. 게이트는 명시된 곳에만, 나머지는 끝까지."*
...
- 프론트매터: name / description / disable-model-invocation / argument-hint / allowed-tools. 인자 참조는 위치 변수 `$0`·`$1`(또는 `$ARGUMENTS` 전체), 첫머리 `` !`glab issue view $0` ``으로 이슈 본문 자동 주입.
- 밀도의 기계 상한 = **줄당 500자**(줄 수 상한 없음 — `scripts/check-density.sh`. CI `density-check`는 `allow_failure` = 경고).
