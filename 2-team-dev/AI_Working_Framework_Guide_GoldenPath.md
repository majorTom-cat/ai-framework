...
- **킷 실물 = `scripts/gen-module.cjs`**(인라인 템플릿 — `npm run gen:module <이름>`). Plop·Hygen 등 외부 생성기도 검토했으나 이 규모엔 의존성 없는 자체 스크립트로 충분하다는 결론.
- **방식**: 본보기 모듈에서 템플릿을 추출하고 이름 치환자만 심는다. "본보기 → 템플릿 → 새 모듈"이라 본보기가 바뀌면 템플릿도 같이 고친다(둘이 어긋나면 생성기가 거짓말을 하게 됨).
...
### 4.1 팀 스킬 목록 (현재 19종: 13 개발 + 6 공통 개발자 — **최신 목록·설명의 정본 = repo README §4 표.** 최소로 시작해 실제 필요가 생긴 것만 추가된 결과)

대표 몇 개만 발췌 — 절차·게이트의 감을 잡는 용도다:

| 스킬 | 하는 일 | 사람 게이트 |
| --- | --- | --- |
| `/card <한 줄>` | 카드 등록 — 가장 잦은 자유 지시("카드로 등록해줘")를 절차로 승격 | 등록 전 미리보기 OK |
| **`/dev <카드>`** | 기능 개발 전체 파이프라인 (아래 4.2) | 스펙 승인 · 계획 go (작은 카드는 묶어서 1회) |
| **`/fix <카드>`** | 작은 수정용 경량판 — 스펙·계획 단계 생략, 바로 구현→테스트→검증 | 없음 |
| `/done <카드>` | 마무리 루틴 (화면 확인 단계가 /inspect 를 사용) | CI가 게이트 — 고위험은 수동 게이트+증적 |
...
> **운영 원칙 — 일은 스킬로만 진입한다** (건별 자연어 지시로 진행하지 않는다 — 그래야 전원이 같은 절차를 탄다). 이를 위해 스킬은 두 등급으로 설계돼 있다: **자동 실행 가능**(조회·안내·등록류 — AI가 자연어 지시를 받아 스스로 실행)과 **사람 입력 전용**(`disable-model-invocation: true` — 머지·구현처럼 상태를 크게 바꾸는 절차라 사람의 명시적 명령이 트리거여야 하고, AI가 절차를 임의로 흉내 내는 것은 CLAUDE.md가 금지). **어느 스킬이 어느 등급인지의 정본 = 각 스킬 frontmatter와 repo README §4 표.** 자연어는 질문·탐색·장애 진단에만 남는다.
...
- **사람 게이트는 최대 2개** (스펙 승인·계획 go) — 그 사이는 연속 실행. **작은 카드**(①질문 0건 ②작업 3개 이하 ③남이 쓰는 계약 불변 ④정본 해석 불변)는 둘을 묶어 **1회**로 선다. `고위험` 라벨은 배제 사유가 **아니다**(2026-08-28) · **사용자 본인이 쓴 카드는 ①④만 본다**(2026-09-08)(정본 = `/dev` 2단계 '묶음 게이트'). 성공 사례들의 공통 교훈: *"'계속할까요?' 남발이 시간을 죽인다. 게이트는 명시된 곳에만, 나머지는 끝까지."*
...
킷 실물은 **스킬당 `SKILL.md` 단독**이 기본이다(예: `.claude/skills/dev/SKILL.md`) — 진행 체크리스트·스펙 양식·"본보기를 먼저 읽어라" 지시를 본문에 내장했고, 함정 모음이 커진 스킬만 보조 파일을 곁에 둔다(`done/함정.md`·`ui/시안.md` 등).

- 프론트매터: name / description / disable-model-invocation / argument-hint / allowed-tools / **effort**(배정표 = `_reference/skill-tiers.md` — «가볍게»같은 부사 금지, 값을 적는다) / model. 인자 참조는 위치 변수 `$0`·`$1`(또는 `$ARGUMENTS` 전체), 첫머리 `` !`glab issue view $0` ``으로 이슈 본문 자동 주입.
- 밀도의 기계 상한 = **줄당 500자 + 파일당 5,376자**(`scripts/check-density.sh`·`scripts/check-skill-size.sh`. 파일 상한은 컴팩션이 스킬의 앞 5,000토큰만 남기는 실측에서 왔다 — 넘치면 뒤가 잘리므로 보조 파일로 빼고 **안내를 맨 앞**에 둔다. CI `density-check` 는 `allow_failure` = 경고).
...
| 스킬 하나가 비대해짐 | 참조 파일로 분리 — 비대한 스킬은 오히려 준수율을 떨어뜨림 (기계 상한은 §4.3의 줄당 500자 — "500줄"로 착각하지 마라) |
...
| 층 | 장치 |
| --- | --- |
| 경계 | 킷 자체 검사기 `scripts/check-boundaries.cjs`(v2 — import 경계+DB 경계+순환 의존) — "모듈은 자신과 shared만 import" |
...
| 스킬 | 스킬이 10종 이상으로 자라면 **설명·트리거 충돌을 기계로 검사** (서로 트리거를 잠식하기 시작함) — agent-skills의 evals Tier 2 스크립트(MIT)를 가져다 쓰면 됨 (`4-reference/agent-skills-analysis.md` §6(프레임워크 홈 — 킷에 동봉되지 않는다)). 충돌이 문제 되는 건 *자동 실행 가능* 등급 사이뿐이다(등급 정본 = §4.1 운영 원칙의 포인터) — **다음 자동 실행 스킬이 추가되는 시점에 evals 도입** |
